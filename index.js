...
class Store {
  ...
  unsubscribe(customer) {
    this.customers = this.customers.filter((cut) => cut !== customer);
  }
  /**
  ...
  sendNotify(product) {
    ...
    this.customers.forEach((subscriber) =>
...
class Writer {
  ...
  // Він викликає статичний метод класу Version, create, передаючи йому поточний текст як аргумент.content
  #store() {
